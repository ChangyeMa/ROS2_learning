...
bool my_robot_interfaces__msg__led_states__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // states
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      if (!rosidl_runtime_c__int64__Sequence__init(&(ros_message->states), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create int64__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      int64_t * dest = ros_message->states.data;
...
PyObject * my_robot_interfaces__msg__led_states__convert_to_py(void * raw_ros_message)
{
  ...
  {  // states
    ...
    if (ros_message->states.size > 0) {
      ...
      int64_t * src = &(ros_message->states.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->states.size * sizeof(int64_t));
      assert(data != NULL);
